...
def get_scene_ids_aoi(site_aois, start_date, end_date, cc, collections, api_key):
    ...
    aois_scene_ids = [{
        aoi: [[f['id'], f['geometry']['coordinates']]
              for f in results_PS[aoi]['features']]
...
def parse_arg(args):
    ...
    parse0.add_argument(
        '--start_date', help="start date in format of: year, month, day")
    parse0.add_argument(
        '--end_date', help="start date in format of: year, month, day")
    parse0.add_argument(
        '--cloud_cover', help='cloud cover in float, e.g. 0.05 for under 5%')
    parse0.add_argument('--zoom', help='OSM zoom level, e.g. 16')
    parse0.add_argument(
        '--out_tex', help='txt name to save all the output tiles')
