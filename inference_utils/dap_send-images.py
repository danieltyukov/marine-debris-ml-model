...
def main(files):
    ...
    queue = sqs.Queue(
        '	https://sqs.us-east-1.amazonaws.com/552819999234/covidmlTileQueue')
    for tile_ind_file in files:
       ...
       # Get tiles from text file if pre-computed
        with open(tile_ind_file, 'r') as tile_file:
            tile_inds = [ind.strip().split('-')
                         for ind in tile_file if len(ind)]
        batch_size = 10
        ...
        print('Found {} tiles, pushing to SQS Queue: {}'.format(
            len(tile_inds), queue.url))
...
def parse_arg(args):
    ...
    parse0.add_argument('--inds_txt_files', nargs='*',
                        help='txt files for tiles index from geodex')
